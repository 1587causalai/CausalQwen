...
该模块将混合了文本和数值的原始输入，转化为统一的、数值感知的特征向量序列。这个过程的核心挑战在于，如何在不干扰基座模型（Qwen）强大的原生分词能力的前提下，稳健地识别和分离出数值。我们通过一个精巧的"三步走"策略来实现这一点：

-   **步骤 1：精确预处理与数值提取**
    在调用Qwen分词器之前，我们首先使用一个高度精确的正则表达式对输入文本进行扫描：
    `(?<![a-zA-Z0-9_])(-?\d+(\.\d+)?)(?![a-zA-Z0-9_])`
    这个表达式利用"否定式前后查找"技术，确保只匹配独立的数字（如 `price is 50.5`），而不会错误地匹配嵌入在单词中的数字（如 `Qwen2.5`）。
    - **提取**: 所有匹配到的数值（如 `50.5`, `-10`）被解析为浮点数并暂存入一个列表中。
    - **替换**: 原始文本中对应的数字被替换为一个临时的、独特的占位符（`_NUM_HOLDER_`）。

-   **步骤 2：调用原生分词器**
    经过预处理的文本（现在包含了占位符而非实际数字）被送入原生的Qwen分词器。分词器会将其高效、准确地切分为词元序列，并将 `_NUM_HOLDER_` 视为一个不可再分的整体。

-   **步骤 3：后处理与张量组装**
    在获得分词结果（`input_ids`）后，我们进行最后一次"狸猫换太子"操作：
    - **ID 替换**: 在 `input_ids` 张量中，所有等于占位符ID的词元，其ID被替换为我们的特殊 `<NUM>` 词元ID。
    - **数值对齐**: 与此同时，我们根据第一步暂存的数值列表，按顺序将这些真实的数值填充到 `numerical_values` 张量的相应位置。

通过这种"**识别-占位-分词-替换**"的解耦流程，我们既利用了正则表达式在模式匹配上的灵活性，又完全保留了基座模型分词的稳定性和性能，从而实现了对数值的稳健处理。

-   **增强嵌入**: 对于每个位置 $i$，计算增强的词元嵌入：
    $$e_i = \text{embed}(x_i) + \phi(v_i)$$
    其中，$\text{embed}(x_i)$ 是词元的基础嵌入（对于`<NUM>`词元，它是其自身的嵌入），$\phi(v) = \text{sign}(v) \cdot \ln(1 + |v|) \cdot \vec{e}$ 是数值编码函数，$\vec{e}$ 是归一化的方向向量。
...
CausalQwen 提供多种推理模式，核心区别在于如何处理不确定性。本章首先介绍所有模式共用的自回归生成框架，然后详细阐述各种具体的推理策略。

### 5.1 对话生成：自回归循环 (Dialogue Generation: The Autoregressive Loop)

为了能像标准大语言模型一样进行对话，CausalQwen 采用自回归（Autoregressive）的方式生成文本。这意味着模型逐个生成词元，并将新生成的词元添加回输入，用于预测下一个词元。这个循环不断进行，直到触发停止条件（如生成了特殊的终止符或达到最大长度）。

CausalQwen 的独特之处在于，在每个生成步骤中，它都需要决策是输出一个词汇表中的标准词元，还是一个具体的数值。

**在每个生成步骤 `i` 的决策流程如下 (参见附录 图 6)**：

1.  **执行前向传播**：将当前已生成的序列 $(x_1, \dots, x_{i-1})$ 作为输入，模型执行一次完整的前向传播，得到当前位置的分类与回归决策分布参数 (`loc_S`, `scale_S`, `loc_Y`, `scale_Y`)。

2.  **选择候选词元**：利用某种具体的推理策略（详见 5.2-5.5 节），从分类决策分布中选择一个候选词元 ID，记为 $\hat{y}_{\text{cls},i}$。例如，在最简单的确定性模式下，$\hat{y}_{\text{cls},i} = \arg\max_k P_{k,i}$。

3.  **最终决策与输出**：
    *   **如果 $\hat{y}_{\text{cls},i}$ 是 `<NUM>` 词元的 ID**：模型决定输出一个数值。此时，该步骤的最终输出是回归头预测的**具体数值** $\hat{y}_{\text{reg},i} = \text{loc}_{Y_i}$。
    *   **如果 $\hat{y}_{\text{cls},i}$ 是任何其他词元 ID**：模型决定输出一个标准词元。此时，该步骤的最终输出就是这个**词元 ID** $\hat{y}_{\text{cls},i}$ 本身。

4.  **追加并重复**：将上一步的输出（无论是词元ID还是一个代表数值的占位符）追加到序列末尾，形成新的输入，然后返回第 1 步，开始预测下一个位置 `i+1`。

5.  **最终文本重构**：当生成循环结束后，模型会得到一个由"词元ID"和"具体数值"混合组成的序列。一个后处理（detokenization）步骤会将这个序列转换为最终的人类可读文本。例如，一个混合序列 `['价格', '是', 99.9, '元']` 会被重构为字符串 `"价格是99.9元"`。

通过这个机制，CausalQwen 能够将数值计算无缝地编织到标准的文本生成流程中，从而实现流畅、信息丰富的对话。

### 5.2 推理模式一：确定性推理 (Deterministic Inference)

这是默认的、最高效的模式，完全基于解析计算，不涉及随机采样。在此模式下，上述循环第二步的候选词元选择方式为：

-   **分类预测**: $\hat{y}_{\text{cls},i} = \arg\max_k P_{k,i}$
-   **回归预测**: $\hat{y}_{\text{reg},i} = \text{loc}_{Y_i}$ (柯西分布的中位数，最稳健的点估计)

### 5.3 推理模式二：因果采样 (Causal Sampling)

这是一种新颖的"原因采样"范式，不直接对"结果"采样，而是对"原因"（个体因果表征 U）采样。在此模式下，第二步的操作变为：

1.  **采样"原因"**: 根据推断出的分布 $\text{Cauchy}(\text{loc}_{U_i}, \text{scale}_{U_i})$，采样一个**具体的**表征向量 $u_i$。
2.  **观察"结果"**: 将这个确定的 $u_i$ 传入**行动网络**，得到**确定性的**分类分数 $s_{k,i}$ 和回归值 $y_i$。
3.  **选择候选词元**: $\hat{y}_{\text{cls},i} = \arg\max_k s_{k,i}$。

### 5.4 推理模式三：兼容传统采样 (Compatibility with Traditional Sampling)

CausalQwen 完全兼容传统 `top-k`/`top-p` 采样。行动网络输出的 `loc_S` (形状: `[B, S, V_full]`) 可以被直接视作标准语言模型输出的 logits。在此模式下，第二步操作为：

1.  **计算概率**:
    $$
    P_{\text{softmax}}(y_i=k|x) = \frac{\exp(\text{loc}_{S_{k,i}})}{\sum_{j=1}^{V_{\text{full}}} \exp(\text{loc}_{S_{j,i}})}
    $$
2.  **执行采样**: 在这组概率上执行标准的 `top-k`/`top-p` 采样，得到候选词元 $\hat{y}_{\text{cls},i}$。

### 5.5 (高级) 共享随机性：因果采样的高级模式
...
这张图描绘了 CausalQwen 在推理时，如何通过自回归循环，在每一步决策是生成一个标准词元还是一个具体的数值。

```mermaid
graph TD
    subgraph "自回归循环 (在步骤 i )"
        direction TB
        A["输入: 已生成序列<br>(x₁...xᵢ₋₁)"] --> B{"执行前向传播"};
        B --> C["获得当前位置的<br>分类与回归分布"];
        
        C --> D{"选择候选词元 ID<br>ŷ_cls,i<br>(依据某种推理模式)"};
        
        D --> E{"决策<br>Is ŷ_cls,i == <NUM_ID>?"};
        
        E -- "是" --> F["输出 = 回归值<br>loc_Y,i"];
        E -- "否" --> G["输出 = 词元 ID<br>ŷ_cls,i"];
