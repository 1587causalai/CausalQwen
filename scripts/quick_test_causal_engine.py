...
1. 数据生成与处理
   ┌─────────────────┐    ┌──────────────────┐    ┌─────────────────────┐
   │ 生成原始数据     │ -> │ 训练/测试分割     │ -> │ 异常标签注入         │
   │ make_regression │    │ train_test_split │    │ inject_shuffle_noise│
   │ make_classification│  │                  │    │ (仅训练集)          │
   └─────────────────┘    └──────────────────┘    └─────────────────────┘
                                   │                         │
                                   v                         v
   ┌─────────────────────────────────────────────────────────────────────────┐
...
2. 模型训练 (在标准化空间中)
   ...
   │                                                                          │
   │  ┌─────────────────┐  ┌─────────────────┐  ┌─────────────────────────┐   │
   │  │ sklearn MLP     │  │ PyTorch MLP     │  │ CausalEngine           │   │
   │  │ (基线模型)       │  │ (PyTorch基线)   │  │ - deterministic mode   │   │
   │  │                 │  │                 │  │ - standard mode        │   │
   │  └─────────────────┘  └─────────────────┘  └─────────────────────────┘   │
   │           │                      │                       │               │
   │           └──────────────────────┼───────────────────────┘               │
...
3. 模型评估 (转换回原始尺度)
   ...
   │  评估阶段: 在干净的原始尺度数据上测试                                     │
   │  ┌─────────────────┐     ┌──────────────┐     ┌─────────────────┐        │
   │  │ 标准化空间预测   │ --> │ 逆标准化变换  │ --> │ 原始尺度评估     │        │
   │  │ model.predict   │     │ scaler.inverse│     │ 在干净测试集     │        │
   │  │ (scaled_input)  │     │ _transform    │     │ 上计算指标       │        │
   │  └─────────────────┘     └──────────────┘     └─────────────────┘        │
   │                                                                          │
   │  这样设计的原因:                                                         │
   │  • 训练时挑战模型的抗噪能力                                              │
