...
在理论上，模型可以从任意随机状态开始学习。然而在实践中，一个糟糕的初始状态可能导致损失爆炸和训练失败。为了解决这个问题，我们设计了一套基于"知识转移"的初始化策略。其核心思想是：在训练开始的`t=0`时刻，我们模型的行为应该等价于预训练的Qwen模型，同时对新增的模块（如回归和`<NUM>`预测）持有合理的、基于先验知识的"偏见"。
