...
**噪声强度对比实验设计**:
为理解噪声如何影响模型表现，我们将测试四个关键的固定 `b_noise` 值：
| `b_noise` 值 | 含义 | 预期效果 |
| :--- | :--- | :--- |
| `0.0` | 无噪声 | 极高置信度，可能严重过拟合 |
| `0.1` | **标准噪声（默认）** | 平衡的不确定性表达 |
| `1.0` | 高噪声 | 保守预测，高度不确定性 |
| `10.0` | 极高噪声 | 极度保守，可能欠拟合 |

通过这组对比实验，我们可以：
1. 观察从确定性到高不确定性的完整性能谱
2. 理解噪声的数量级效应和临界阈值
3. 验证噪声作为正则化机制的有效范围
...
1. **噪声强度影响分析**：
   - 对比实验组A中不同固定噪声值（0.0, 0.1, 1.0, 10.0）的性能
   - 绘制性能-噪声曲线，分析数量级变化的效应
...
3. **方法论层面的认知**：固定噪声实验的结果为自适应学习提供了明确的参照系。我们可以回答：
   - 模型自主学到的噪声强度是否落在关键区间（0.0-10.0）内？
   - 自适应学习是否真的优于手动选择的最优固定值？
   - 不同数据集是否需要不同数量级的噪声强度，这是否反映了任务的内在特性？
